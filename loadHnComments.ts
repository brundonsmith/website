...
const COMMENTS_CACHE = new Map<
  string,
  { data: PostComments | null; timestamp: number }
>()
...
export default async function getCommentsCached(postName: string) {
  ...
  if (!cached) {
    ...
    if (
      !POST_COMMENTS_LOADING.get(postName) &&
      (Date.now() - cached.timestamp > COMMENTS_CACHE_LIFETIME)
    ) {
...
async function getPostId(postName: string) {
  return await fetch(
    `https://hn.algolia.com/api/v1/search?tags=story&query=${
      encodeURIComponent('brandons.me/blog/' + postName)
    }`,
  )
    .then((res) => res.json())
    ...
    await fetch(
      `https://hn.algolia.com/api/v1/search?tags=story&query=${
        encodeURIComponent('brandonsmith.ninja/blog/' + postName)
      }`,
    )
      .then((res) => res.json())
      .then((res) => res.hits[0]?.objectID)
...
async function getCommentData(id: string) {
  ...
  return {
    ...
    time,
    kids: kids &&
      (await Promise.all(kids.map(getCommentData))).filter((item) => item.text),
  }
}
...
function renderComments({ id, by, text, time, kids }: Comment): string {
  ...
  return html`
        <div class="comment-heading ${isMe ? 'me' : ''}">
            ...
            ${
    text
      ? `<a class="by" href="https://news.ycombinator.com/user?id=${by}" target="_blank">
                ${by ?? ''}
            </a>`
      : ''
  }
            ...
            ${
    text
      ? `<a class="time" href="https://news.ycombinator.com/item?id=${id}" target="_blank">
                ${ago(date)}
            </a>`
      : ''
  }
